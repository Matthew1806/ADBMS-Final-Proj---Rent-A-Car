...
// Homepage and cars-page micro interactions
document.addEventListener('DOMContentLoaded', function() {
    ...
    const resetButton = document.getElementById('resetCarFilters');
    const resetInlineButton = document.getElementById('resetCarFiltersInline');
    const resultCount = document.getElementById('carsResultCount');
    const noResultsBlock = document.getElementById('carsNoResults');
    const carCards = document.querySelectorAll('.car-filter-item');
    ...
    const applyFilters = () => {
        ...
        const selectedTransmission = transmissionFilter?.value || 'all';
        ...
        carCards.forEach((card) => {
            ...
            const rawPrice = String(card.dataset.price || '0');
            ...
            const matchesFuel = selectedFuel === 'all' || fuel.includes(selectedFuel);
            const matchesMin = !minPrice || price >= minPrice;
            const matchesMax = !maxPrice || price <= maxPrice;

            const visible = matchesQuery && matchesBrand && matchesTransmission && matchesFuel && matchesMin && matchesMax;
        ...
        if (resultCount) {
            resultCount.textContent = `Showing ${shown} car${shown === 1 ? '' : 's'}`;
        }

        if (noResultsBlock) {
            noResultsBlock.style.display = shown === 0 ? 'block' : 'none';
        }
    };

    [searchInput, brandFilter, transmissionFilter, fuelFilter, minPriceInput, maxPriceInput]
        .filter(Boolean)
        .forEach((el) => {
...
// Booking form UX enhancements: inline validation and transparent summary
document.addEventListener('DOMContentLoaded', function() {
    ...
    function setInlineError(field, message) {
        if (!field) return;
        let errorEl = field.parentElement.querySelector('.client-error');
        if (!errorEl) {
            errorEl = document.createElement('div');
            errorEl.className = 'error client-error';
    ...
    function clearInlineError(field) {
        if (!field) return;
        const errorEl = field.parentElement.querySelector('.client-error');
    ...
    function validateName() {
        ...
        if (value.length < 10) {
            setInlineError(nameInput, 'Please enter at least 10 characters for full name.');
            return false;
        }
    ...
    function validateContact() {
        ...
        const value = contactInput.value.replace(/\s+/g, '');
        const phonePattern = /^(\+63|0)\d{10}$/;
        if (!phonePattern.test(value)) {
            setInlineError(contactInput, 'Use PH format like 09XXXXXXXXX or +63XXXXXXXXXX.');
            return false;
        }
        clearInlineError(contactInput);
        return true;
    }

    function updateSummary() {
        if (!summaryCar || !summaryDates || !summaryDays || !totalDisplay) return;

        const selectedOption = carSelect && carSelect.selectedOptions ? carSelect.selectedOptions[0] : null;
        const selectedCar = selectedOption && selectedOption.value ? selectedOption.textContent.split(' - ')[0] : 'Not selected';
        summaryCar.textContent = selectedCar;

        if (!pickupInput.value || !returnInput.value) {
            summaryDates.textContent = 'Not set';
        ...
        summaryDates.textContent = `${pickupInput.value} to ${returnInput.value}`;
        summaryDays.textContent = String(days);
    }

    if (nameInput) nameInput.addEventListener('input', validateName);
    if (emailInput) emailInput.addEventListener('input', validateEmail);
    if (contactInput) contactInput.addEventListener('input', validateContact);

    [carSelect, pickupInput, returnInput].forEach((field) => {
        if (field) field.addEventListener('change', updateSummary);
    });

    bookingForm.addEventListener('submit', function(e) {
        const checks = [validateName(), validateEmail(), validateContact()];
        if (checks.some((ok) => !ok)) {
            e.preventDefault();
        }
    });

    // Keep summary and total aligned on initial load.
    updateSummary();
});
